...
class ProfileRunParams():
    ...
    def run_profmerge(self) -> None:
        ...
        self.get_params().write_text(
            cov.ReflectionCLI(
                profdata=cov.ProfdataConfig(
                    build_profile_dir=str(self.get_summary()),
                ...
                output=str(self.get_sqlite()),
                mode=cov.Mode.RunProfileMerge,
            ).model_dump_json(indent=2))
...
def test_base_run(stable_test_dir: Path) -> None:
    dir = stable_test_dir
...
@pytest.mark.test_release
def test_coverage_regions_multiple_contexts(stable_test_dir: Path) -> None:
    dir = stable_test_dir
    cmd = ProfileRunParams(
        dir=dir,
        main="main.cpp",
        files={
            "main.cpp": corpus_base.joinpath("test_coverage_regions_1.cpp").read_text()
        },
        run_contexts={
            "function_3": [],
            "function_1": ["2"],
...
@pytest.mark.test_release
def test_region_types(stable_test_dir: Path) -> None:
    dir = stable_test_dir
    cmd = ProfileRunParams(
        dir=dir,
        main="main.cpp",
...
def test_region_types(stable_test_dir: Path) -> None:
    ...
    cmd = ProfileRunParams(
        dir=dir,
        main="main.cpp",
        files={"main.cpp": corpus_base.joinpath("test_macro_expansions.cpp").read_text()})
...
@pytest.mark.test_release
def test_file_coverage_filter(stable_test_dir: Path) -> None:
    dir = stable_test_dir
    cmd = ProfileRunParams(
        dir=dir,
        main="main.cpp",
...
@pytest.mark.test_release
def test_file_segmentation_1(stable_test_dir: Path) -> None:
    dir = stable_test_dir
...
@pytest.mark.test_release
def test_file_segmentation_2(stable_test_dir: Path) -> None:
    dir = stable_test_dir
...
def test_coverage_annotation_single_run(stable_test_dir: Path) -> None:
    dir = stable_test_dir
    code = "\n".join([
        "",
        "int main() {",
        ...
        to_debug_json(
            dict(Lines=[
                ...
                dict(Segments=[
                    ...
                    dict(Text='main', Annotations={}, TokenKind='Token.Name.Function'),
                    dict(Text='()', Annotations={}, TokenKind='Token.Punctuation'),
                    dict(Text=' ', Annotations={}, TokenKind='Token.Text.Whitespace'),
                    dict(Text='{', CoverageSegmentIdx=[0], TokenKind='Token.Punctuation'),
                    dict(Text='\n',
...
@pytest.mark.test_release
@pytest.mark.test_coverage_annotation_file_cxx
def test_coverage_annotation_multiple_run_single_segment(stable_test_dir: Path) -> None:
...
@pytest.mark.test_release
@pytest.mark.test_coverage_annotation_file_cxx
def test_coverage_annotation_multiple_run_multiple_segment(stable_test_dir: Path) -> None:
    dir = stable_test_dir
...
@pytest.mark.test_release
@pytest.mark.test_coverage_annotation_file_cxx
def test_template_coverage_annotations(stable_test_dir: Path) -> None:
    dir = stable_test_dir
...
@pytest.mark.test_release
@pytest.mark.test_coverage_annotation_file_cxx
def test_macro_coverage1(stable_test_dir: Path) -> None:
    dir = stable_test_dir
...
@pytest.mark.test_release
@pytest.mark.test_coverage_annotation_file_cxx
def test_exporter_tcc_coverage(stable_test_dir: Path) -> None:
    dir = stable_test_dir
