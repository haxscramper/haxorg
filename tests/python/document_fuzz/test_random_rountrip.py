...
def build_subnodes(draw: st.DrawFn, ctx: OrgGenCtx):
    return st.lists(node_strategy(ctx=ctx))


@st.composite
def build_Document(draw: st.DrawFn, ctx: OrgGenCtx):
    return draw(st.builds(org.Document,))
...
def node_strategy(draw, ctx: OrgGenCtx):
    ...
    match item:
        case osk.StmtList: return draw(build_StmtList(ctx=ctx))
        case osk.Empty: return draw(build_Empty(ctx=ctx))
        case osk.Cell: return draw(build_Cell(ctx=ctx))
        case osk.Row: return draw(build_Row(ctx=ctx))
        case osk.Table: return draw(build_Table(ctx=ctx))
        case osk.HashTag: return draw(build_HashTag(ctx=ctx))
        case osk.Footnote: return draw(build_Footnote(ctx=ctx))
        case osk.Completion: return draw(build_Completion(ctx=ctx))
        case osk.Paragraph: return draw(build_Paragraph(ctx=ctx))
        case osk.Center: return draw(build_Center(ctx=ctx))
        case osk.Caption: return draw(build_Caption(ctx=ctx))
        case osk.CmdName: return draw(build_CmdName(ctx=ctx))
        case osk.CmdResults: return draw(build_CmdResults(ctx=ctx))
        case osk.CommandGroup: return draw(build_CommandGroup(ctx=ctx))
        case osk.Tblfm: return draw(build_Tblfm(ctx=ctx))
        case osk.Quote: return draw(build_Quote(ctx=ctx))
        case osk.Verse: return draw(build_Verse(ctx=ctx))
        case osk.Example: return draw(build_Example(ctx=ctx))
        case osk.CmdArguments: return draw(build_CmdArguments(ctx=ctx))
        case osk.CmdAttr: return draw(build_CmdAttr(ctx=ctx))
        case osk.CmdArgument: return draw(build_CmdArgument(ctx=ctx))
        case osk.Export: return draw(build_Export(ctx=ctx))
        case osk.AdmonitionBlock: return draw(build_AdmonitionBlock(ctx=ctx))
        case osk.Call: return draw(build_Call(ctx=ctx))
        case osk.Code: return draw(build_Code(ctx=ctx))
        case osk.Time: return draw(build_Time(ctx=ctx))
        case osk.TimeRange: return draw(build_TimeRange(ctx=ctx))
        case osk.Macro: return draw(build_Macro(ctx=ctx))
        case osk.Symbol: return draw(build_Symbol(ctx=ctx))
        case osk.SubtreeLog: return draw(build_SubtreeLog(ctx=ctx))
        case osk.Subtree: return draw(build_Subtree(ctx=ctx))
        case osk.InlineMath: return draw(build_InlineMath(ctx=ctx))
        case osk.Escaped: return draw(build_Escaped(ctx=ctx))
        case osk.Newline: return draw(build_Newline(ctx=ctx))
        case osk.Space: return draw(build_Space(ctx=ctx))
        case osk.Word: return draw(build_Word(ctx=ctx))
        case osk.AtMention: return draw(build_AtMention(ctx=ctx))
        case osk.RawText: return draw(build_RawText(ctx=ctx))
        case osk.Punctuation: return draw(build_Punctuation(ctx=ctx))
        case osk.Placeholder: return draw(build_Placeholder(ctx=ctx))
        case osk.BigIdent: return draw(build_BigIdent(ctx=ctx))
        case osk.Bold: return draw(build_Bold(ctx=ctx))
        case osk.Underline: return draw(build_Underline(ctx=ctx))
        case osk.Monospace: return draw(build_Monospace(ctx=ctx))
        case osk.MarkQuote: return draw(build_MarkQuote(ctx=ctx))
        case osk.Verbatim: return draw(build_Verbatim(ctx=ctx))
        case osk.Italic: return draw(build_Italic(ctx=ctx))
        case osk.Strike: return draw(build_Strike(ctx=ctx))
        case osk.Par: return draw(build_Par(ctx=ctx))
        case osk.List: return draw(build_List(ctx=ctx))
        case osk.ListItem: return draw(build_ListItem(ctx=ctx))
        case osk.Link: return draw(build_Link(ctx=ctx))
        case osk.DocumentOptions: return draw(build_DocumentOptions(ctx=ctx))
        case osk.Document: return draw(build_Document(ctx=ctx))
        case osk.ParseError: return draw(build_ParseError(ctx=ctx))
        case osk.FileTarget: return draw(build_FileTarget(ctx=ctx))
        case osk.TextSeparator: return draw(build_TextSeparator(ctx=ctx))
        case osk.Include: return draw(build_Include(ctx=ctx))
        case osk.DocumentGroup: return draw(build_DocumentGroup(ctx=ctx))
        case _:
            assert False, item
