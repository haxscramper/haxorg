...
def parse_google_tests(binary_path: Path) -> list[GTestParams]:
    ...
    for suite in model.testsuites:
        for method in suite.testsuite:
            # FIXME: `method.value_param` contains JSON value only for parametrized tests
            # generated by the `CorpusAllParametrized`, other tests may have different
            # parameter representation.
            if method.value_param and isinstance(json.loads(method.value_param), dict):
