#include <haxorg/test/corpusrunner.hpp>
#include <haxorg/sem/perfetto_org.hpp>
#include <hstd/ext/perfetto_aux_impl_template.hpp>
#include <hstd/stdlib/JsonSerde.hpp>
#include <hstd/stdlib/YamlSerde.hpp>
#include <hstd/stdlib/ColTextSerde.hpp>
#include <hstd/stdlib/IntSetSerde.hpp>
#include <hstd/stdlib/MapSerde.hpp>

using namespace org::parse;
using namespace org::test;


template <>
struct hstd::JsonSerde<TestResult::Data> {
    static json to_json(TestResult::Data const& data) {
        json j    = json::object();
        j["kind"] = fmt1(TestResult::getKind(data));
        std::visit(
            overloaded{
                [&](TestResult::Fail const& file) { j["fail"] = to_json_eval(file); },
                [&](TestResult::Success const& file) {
                    j["success"] = to_json_eval(file);
                },
                [&](TestResult::Skip const& file) { j["skip"] = to_json_eval(file); },
            },
            data);
        return j;
    }
};

int main(int argc, char** argv) {
    LOGIC_ASSERTION_CHECK(argc == 2, "");
    auto spec = hstd::JsonSerde<ParseSpec>::from_json(json::parse(std::string(argv[1])));

    TestResult result      = gtest_run_spec(TestParams{.spec = spec}, "/tmp");
    json       json_result = hstd::to_json_eval(result);
    std::cout << json_result.dump() << std::endl;
    return 0;
}
