...
class ProtoBuilder:
    ...
    def build_protobuf(self) -> BlockId:
        ...
        def aux_field(
            it: codegen_ir.GenTuField, indexer: Generator[int], indent: int
        ) -> BlockId:
        ...
        def aux_enum(
            parent: codegen_ir.GenTuEnum,
            it: codegen_ir.GenTuEnumField,
            idx: int,
            indent: int,
        ) -> BlockId:
        ...
        def aux_field_list(
            fields: Iterable[codegen_ir.GenTuField], indexer: Generator[int], indent: int
        ) -> Iterable[BlockId]:
        ...
        def aux_item(
            it: codegen_ir.GenTuEntry | codegen_ir.GenTuField, indent: int
        ) -> Optional[BlockId]:
            match it:
            ...
            [
                braced(
                    ...
                    aux_field_list(
                        (
                            codegen_ir.GenTuField(
                                Name=rec.Name.Name.lower(), Type=rec.Name
                            )
            ...
            list(
                itertools.chain(
                    *(
                        [it, self.t.text("")]
                        for it in drop_none(
                            aux_item(it, indent=0) for it in self.types_list
                        )
                    )
                )
            )
    ...
    def get_all_concrete_types(self) -> Iterable[codegen_ir.GenTuStruct]:
        return (
            ...
            if isinstance(rec, codegen_ir.GenTuStruct)
            and not rec.IsAbstract
            and rec.Name.isOrgType()
        )
        ...
        match typ:
            ...
            case _:
                if codegen_ir.in_type_list(typ, self.enum_type_list):
                    return QualType.ForName(
                        "_".join(
                            typ.withoutSpace("sem").withoutSpace("org").flatQualName()
                        )
                    ).withExtraSpace("orgproto")
    ...
    def get_field_read_op(
        ...
        if self.is_direct_set_type(field_type):
            read_expr = self.ast.XCallRef(
                proto_ptr, self.sanitize_ident_for_protobuf(field.Name)
            )
    ...
    def build_protobuf_writer_for_field(
        ...
        if flat in self.variant_type_list or field.Type.Name in [
            ...
            if is_typedef:
                ...
                reader_switch = cpp.SwitchStmtParams(
                    Expr=self.ast.XCallRef(
                        t.text(PROTO_VALUE_NAME), self.oneof_field_name(field) + "_case"
                    )
                )
            ...
            if is_typedef:
                assert field.Type is not None, "Field type must not be None"
                kind_type = proto_type.asSpaceFor(field.Type.withoutAllScopeQualifiers())  # type: ignore
            ...
            for idx, var_field in enumerate(
                ...
                var_dot_write = t.line(
                    [t.text(f"std::get<{idx}>("), dot_write, t.text(")")]
                )
                var_dot_read = t.line(
                    [
                        self.ast.XCallRef(
    ...
    def build_protobuf_fields_for_variant(
        ...
        return (
            codegen_ir.GenTuField(
                Name=self.rewrite_for_proto_grammar(sub.withoutAllScopeQualifiers())
                .lower()
                .replace(" ", "_"),
                Type=sub,
            )
            ...
            case codegen_ir.GenTuStruct():
                ...
                for base in codegen_ir.get_base_list(it, self.type_map):
                    ...
                    assert base_type is None or isinstance(
                        base_type, codegen_ir.GenTuStruct
                    ), "Base type must be GenTuStruct or None"
    ...
    def build_protobuf_writer(
        self,
    ) -> Tuple[Iterable[cpp.RecordParams], Iterable[cpp.MethodDefParams]]:
