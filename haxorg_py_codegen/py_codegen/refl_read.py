...
def conv_proto_reflection_params(
    reflection_params: Optional[pb_google.Struct],
) -> codegen_ir.GenTuReflParams:
...
def conv_proto_templates(
    templates: list[pb.TemplateParams],
) -> codegen_ir.GenTuTemplateParams:
            ...
            for proto_param in proto_group.params:
                ...
                group.Params.append(
                    codegen_ir.GenTuTemplateTypename(
                        ...
                        if proto_param.non_type_constraint
                        else None,
                    )
                )
...
@beartype
def conv_proto_record(
    record: pb.Record, original: Optional[Path]
...
@beartype
def conv_proto_function(
    rec: pb.Function, original: Optional[Path]
...
@beartype
def conv_proto_typedef(
    rec: pb.Typedef, original: Optional[Path]
...
@beartype
def conv_proto_include(
    rec: pb.Include, original: Optional[Path]
...
def dict_to_dataclass(
    ...
    def aux_impl(data: Any, cls: type[Any], path: tuple[Any, ...]) -> Any:
        ...
        if origin is dict:
            ...
            return {
                aux(key, key_type, path + (f"<key:{key}>",)): aux(
                    value, value_type, path + (key,)
                )
