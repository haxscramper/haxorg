...
- Strictly defined schema -- strongly typed AST/DOM structure for individual nodes, auto-generated protobuf grammar, use vocabulary types in the data model.
...
| Phase      | Status   | Notes                                                   |
|------------+----------+---------------------------------------------------------|
| C++ core   | [WIP] Alpha | Whole pipeline is complete, working on testing          |
| Python API | [DONE] Alpha | Whole DOM model is exposed, working on API improvements |
| C API      | [DONE] Alpha  | Make the library usable from virtually any PL           |

** COMPLETED [WIP] C++ Core

- [DONE] Machine-readable data model -- describe the data model in machine-readable format and generate C++ code for it
- [DONE] Data transform pipeline -- lexer, parser, sem convert
- [DONE] Document test corpus -- yaml specification for the given/expected files
- [DONE] WIP Org-mode AST back to string -- for document formatting and round-trip PBT testing (make random document, render, parse back, compare with original)

** WIP [WIP] Python API

- [DONE] Automatically wrap C++ data model for python -- done, tested, working on ergonomics improvements
...
- [TODO] Automatic wrapper of the C++ API to C
- [TODO] Machine-readable description of the C API
