...
template <>
struct value_domain<OrgSemPlacement>
...
template <>
struct value_domain<OrgHorizontalDirection>
    : public value_domain_ungapped<
...
template <>
struct value_domain<OrgVerticalDirection>
    : public value_domain_ungapped<
...
template <>
struct value_domain<OrgSpecName>
    : public value_domain_ungapped<
          OrgSpecName,
          OrgSpecName::Unnamed,
...
enum class OrgNodeKind : short int
{
    ...
    None,
    /// \brief Toplevel part of the ast, not created by parser, and only
    /// used in `semorg` stage
    Document,
    ...
    ///
    ///
    ///    Failed node parse - technically there are no /wrong/ syntax in
    ///    the
    ///    org-mode document because everything can be considered a one
    ///    large
    ///    word or a paragraph with flat `Word` content.
    ///
    ///    Error node's extent covers all subnodes that were constructed
    ///    during nested content parsing plus ErrorTerminator node with
    ///    error
    ///    token (description of the parsing failure). So failure node will
    ///    be
    ...
    ///
    ///    Error node can be produced by any parsing routine, although it
    ///    is
    ...
    ///    example, `*** subtree` (and any title variations) can never be
    ///    an
    ///    error in itself. Title /text/ might contain an error, but
    ///    invalid
    ...
    /// together.
    StmtList,
    /// \brief Associated list of statements - AST elements like commands
    /// and links are grouped together if placed on adjacent lines
    AssocStmtList,
    ...
    SubtreeTimes,
    SubtreeStars,
    /// \brief Task compleation cookie, indicated either in percents of
    /// completion, or as `<done>/<todo>` ratio.
    Completion,
    ...
    Bullet,
    ListItem,
    /// \brief Auxilliary wrapper for the paragraph placed at the start of
    ...
    Comment,
    /// \brief Raw string of text from input buffer. Things like particular
    /// syntax details of every single command, link formats are not
    /// handled in parser, deferring formatting to future processing layers
    RawText,
    /// \brief Part of the org-mode document that is yet to be parsed. This
    /// node should not be created manually, it is only used for handling
    ...
    Unparsed,
    /// \brief Undefined single-line command -- most likely custom
    /// user-provided oe
    Command,
    ...
    Result,
    /// \brief regular identifier - `alnum + [-_]` characters for
    ...
    BareIdent,
    /// \brief Big ident used in conjunction with colon at the start of
    /// paragraph is considered an admonition tag: `NOTE: Text`, `WARNING:
    /// text` etc.
    AdmonitionTag,
    ...
    SrcCode,
    /// \brief inline piece of code (such as `src_nim`),. Latter is
    ...
    /// evaluation etc.
    SrcInlineCode,
    /// \brief Call to named source code block. Inline, multiline, or
    /// single-line.
    CallCode,
    /// \brief Passthrough block. Inline, multiline, or single-line. Syntax
    /// is `@@<backend-name>:<any-body>@@`. Has line and block syntax
    /// respectively
    PassCode,
    /// \brief Command arguments
    CmdArguments,
    /// \brief Flag for source code block. For example `-n`, which is used
    /// to to make source code block export with lines
    CmdFlag,
    CmdKey,
    CmdValue,
    ...
    TextSeparator,
    /// \brief Single 'paragraph' of text. Used as generic container for
    /// any place in AST where unordered sentence might be encountered
    /// (e.g. caption, link description) - not limited to actual paragraph
    Paragraph,
    /// \brief Annotated paragraph -- a wrapper around a regular paragraph
    /// kind with added admonition, footnote, list tag prefix and similar
    /// types. `[fn:ID] Some Text` is an annotated paragraph, just like
    /// `NOTE: Text` or `- Prefix :: Body` (in this case list header is an
    /// annotated paragraph)
    AnnotatedParagraph,
    /// \brief Region of text with formatting, which contains standalone
    /// words -
    ///      can itself contain subnodes, which allows to represent nested
    ///      formatting regions, such as `*bold /italic/*` text. Particular
    ///      type
    ///      of identifier is stored in string form in `str` field for
    ///      `OrgNode`
    ///      -- bold is represented as `"*"`, italic as `/` and so on. In
    ///      case
    ///      of explicit open/close pairs only opening one is stored.
    ///      NOTE: when structured sentences are enabled, regular
    ///      punctuation
    ///      elements like `some text (notes)` are also represented as
    ///      `Word,
    ///      Word, Markup(str: "(", [Word])` - e.g. structure is not fully
    ///      flat.
    Bold,
    Italic,
    Verbatim,
    ...
    Monospace,
    Par,
    /// \brief Inline latex math. Contains latex math body - either from
    /// `$dollar-wrapped$` or `\(paren-wrapped\)` inline text.
    InlineMath,
    /// \brief Inline display latex math from `$$double-dollar$$` or
    ...
    Punctuation,
    Colon,
    /// \brief Regular word - technically not different from `orgIdent`,
    ...
    RawLink,
    /// \brief External or internal link. Consists of one or two elements -
    /// target
    ///      (url, file location etc.) and description (`orgParagraph` of
    ///      text).
    ///      Description might be empty, and represented as empty node in
    ///      this
    ///      case. For external links particular formatting of the address
    ///      is
    ///      not handled by parser and instead contains raw string from
    ///      input
    ///      text.
    Link,
    /// \brief Org-mode macro replacement - during export each macro is
    /// expanded
    ///      and evaluated according to it's environment. Body of the macro
    ///      is
    ///      not parsed fully during org-mode evaluation, but is checked
    ///      for
    ///      correct parenthesis balance (as macro might contain elisp
    ///      code)
    Macro,
    /// \brief Raw content to be passed to a particular backend. This is
    /// the most
    ///      compact way of quoting export strings, after `#+<backend>:
    ///      <single-backend-line>` and `#+begin-export <backend>`
    ///      `<multiple-lines>`.
    BackendRaw,
    /// \brief Special symbol that should be exported differently to
    /// various backends - greek letters (`lpha`), mathematical notations
    /// and so on.
    Symbol,
    ...
    StaticInactiveTime,
    DynamicActiveTime,
    /// \brief Single date and time entry (active or inactive),, possibly
    /// with repeater interval. Is not parsed directly, and instead
    /// contains `orgRawText` that can be parsed later
    DynamicInactiveTime,
    /// \brief Date and time range format - two `orgDateTime` entries
    TimeRange,
    /// \brief Result of the time range evaluation or trailing annotation a
    /// subtree
    SimpleTime,
    ...
    Summary,
    /// \brief Org-mode table. Tables can be writtein in different formats,
    /// but in
    ///    the end they are all represented using single ast type. NOTE: it
    ///    is
    ...
    TableRow,
    /// \brief Single cell in row. Might contain anyting, including other
    /// tables, simple text paragraph etc.
    TableCell,
    /// \brief Inline footnote with text placed directly in the node body.
    InlineFootnote,
    /// \brief Footnote entry. Just as regular links - internal content is
    /// not parsed, and instead just cut out verbatim into target AST node.
    Footnote,
    /// \brief Horizotal rule. Rule body might contain other subnodes, to
    /// represnt `---- some text ----` kind of formatting.
    Horizontal,
    /// \brief `#+filetags:` line command
    Filetags,
    /// \brief Original format of org-mode tags in form of `:tagname:`.
    /// Might
    ///    contain one or mode identifgiers, but does not provide support
    ///    for
    ///    nesting - `:tag1:tag2:`. Can only be placed within restricted
    ///    set
    ...
    ///    extension. Can be placed anywere in the document (including
    ///    section
    ///    headers), but does not have separate place in AST (e.g.
    ///    considered
    ...
    /// \brief `@user`
    AtMention,
    /// \brief Custom extension to org-mode. Similarly to `BigIdent` used
    /// to have something like informal keywords `MUST`, `OPTIONAL`, but
    /// instead aimed /specifically/ at commit message headers -
    /// `[FEATURE]`, `[FIX]` and so on.
    BracTag,
    /// \brief Single enclosed drawer like `:properties: ... :end:` or
    /// `:logbook: ... :end:`
    Drawer,
    LatexClass,
    LatexHeader,
    ...
    Property,
    /// \brief Placeholder entry in text, usually writte like `<text to
    /// replace>`
    Placeholder,
...
template <>
struct value_domain<OrgTextContext>
    : public value_domain_ungapped<
          OrgTextContext,
...
{
    None,
    /// \brief MUST This word, or the terms "REQUIRED" or "SHALL", mean
    /// that the definition is an absolute requirement of the
    /// specification.
    Must,
    /// \brief MUST NOT This phrase, or the phrase "SHALL NOT", mean that
    /// the definition is an absolute prohibition of the specification.
    MustNot,
    /// \brief SHOULD This word, or the adjective "RECOMMENDED", mean that
    /// there
    ///    may exist valid reasons in particular circumstances to ignore a
    ///    particular item, but the full implications must be understood
    ///    and carefully weighed before choosing a different course.
    Should,
    /// \brief SHOULD NOT This phrase, or the phrase "NOT RECOMMENDED" mean
    /// that
    ...
    ShouldNot,
    Required,
    /// \brief MAY This word, or the adjective "OPTIONAL", mean that an
    /// item is
...
template <>
struct value_domain<OrgBigIdentKind>
    : public value_domain_ungapped<
          OrgBigIdentKind,
...
enum class OrgTokenKind : short int
{
    ...
    DoubleColon,
    Text,
    /// \brief Unlexed group of statements - used in the list content to
    /// enable secondary parsing.
    StmtList,
    ...
    SubtreeTime,
    SubtreeEnd,
    /// \brief You can write time ranges without any additional formatting
    /// for
    ///    subtrees that have a diary timestamps. For example, you have a
    ///    complex date predicate, but event occurs for `18:00-21:00`, so
    ///    you write it in the random place in the subtree.
    ImplicitTime,
    /// \brief Time duration for the `effort` property or time range length
    /// evaluation
    TimeDuration,
    ...
    StaticTimeRepeater,
    /// \brief Separator dash between two periods in the time range
    /// (`<start>--<finish.`)
    TimeDash,
    ...
    CodeContentEnd,
    /// \brief Uninterrupted text span without newlines - either a whole
    /// line or sub subsection of it if callout or tangle elements were
    /// detected
    CodeText,
    ...
    LinkTargetOpen,
    LinkTargetClose,
    /// \brief No protocol is used in the link, it is targeting some
    /// internal named entry.
    LinkInternal,
    /// \brief Protocol used by the link - `file:`, `https:` etc.
    LinkProtocol,
    /// \brief Full token for the link, used in cases where it does not
    /// make sense to fracture the token - regular https URLs etc.
    LinkFull,
    ...
    TextSeparator,
    /// \brief Fake token inserted by the lexer to delimit start of the
    /// paragraph
    ParagraphStart,
    ParagraphEnd,
    FootnoteStart,
    ...
    Word,
    /// \brief Escaped character in plain text - `\*`, `\/` etc. Escaped
    /// characters and sequences thereof are treated like a regular plain
    /// text.
    Escaped,
    /// \brief Put at the end of the lexer first logbook line to separate
    /// the  note, otherwise is treated as standalone escaped slash.
    DoubleSlash,
    ...
    BigIdent,
    /// \brief Unparsed raw text, either as a part of paragraph or some
    ...
    HashTagClose,
    /// \brief Comma - punctuation or a syntax element (e.g. for macro
    /// arguments)
    Comma,
...
template <>
struct value_domain<OrgCommandKind>
    : public value_domain_ungapped<
          OrgCommandKind,
          OrgCommandKind::None,
...
template <>
struct value_domain<OrgPropertyKind>
    : public value_domain_ungapped<
          OrgPropertyKind,
...
template <>
struct value_domain<OrgUnnumberedKind>
    : public value_domain_ungapped<
          OrgUnnumberedKind,
...
template <>
struct value_domain<OrgBlockLexerState>
    : public value_domain_ungapped<
          OrgBlockLexerState,
