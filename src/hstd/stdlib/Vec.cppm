...
export {
    ...
    /// provides some convienience operators such as `[]` for slices,
    /// *integer* size (less `size() - 1` bugs), more extensive size
    /// checking for accessors (no more `back()`-induced dangling
    /// references that fail five layers down the call stack). Easier
    /// concatenation etc.
    ///
    /// Performance was taken in consideration for these overloads, but
    /// only as long as it did not hinder the useful functionality itself
    /// -- you can append two vectors, check if it `contains()` someting
    /// and so on, even though these operations are \(O(n)\)
    template <typename T>
    class Vec : public std::vector<T> {
      public:
        ...
        /// \brief  Pointwise comparison between vector and any other
        /// indexable container.
        ...
        /// \brief Check if vector has enough elements to access index \arg
        /// idx
        ...
        /// \brief Get reference wrapper to a value at specified index or
        /// empty option if the index is out of range
        ...
        /// \brief 'at' operation for accessing value using backwards
        /// indexing, recommended for use as it unconiditionally does the
        /// bound checking
        ...
        /// \brief Reference to the last element. Checks for proper array
        /// size
        T& back() {
        ...
        /// \brief constant reference to the last element, checks for
        /// vector size
        T const& back() const {
            failEmpty();
            return Base::back();
        ...
        /// \brief Override of the 'back' accessor of the standard vector,
        /// but with check for proper size
        void pop_back() {
            failEmpty();
            Base::pop_back();
        }
        ...
        /// \brief Last accessible index of the vector, useful for slicing
        /// etc.
        ...
        /// \(O(n)\) operation, so better used only on small vectors.
