...
#define __macro_aux(macro, pass, ...) macro(__unpack_pars pass, __VA_ARGS__)
...
#define DESC_FIELDS(classname, arg) BOOST_DESCRIBE_CLASS(classname, (), arg, (), ())
...
#define DEFINE_VISITOR_BASE_STRUCT(                                                      \
    ...
    struct __VisitorTypename<BOOST_PP_TUPLE_REM() __VisitorTypeSpecification> {          \
        static BOOST_PP_TUPLE_REM() __VisitorResultType __VisitorMethodName(             \
            BOOST_PP_TUPLE_REM() __VisitorTypeSpecification const& arg,                  \
            BOOST_PP_TUPLE_REM() __VisitorSharedArgs);                                   \
    };

#define IS_EMPTY_TUPLE(tuple) BOOST_PP_IS_EMPTY(BOOST_PP_TUPLE_ELEM(0, tuple))
...
#define DEFINE_VISITOR_BASE_METHOD_SIGNATURE(                                            \
    ...
    BOOST_PP_TUPLE_REM()                                                                 \
    __VisitorResultType                                                                  \
        __VisitorTypename<BOOST_PP_TUPLE_REM() __VisitorTypeSpecification>::             \
            __VisitorMethodName(                                                         \
                BOOST_PP_TUPLE_REM() __VisitorTypeSpecification const& arg,              \
                BOOST_PP_TUPLE_REM() __VisitorSharedArgs)
