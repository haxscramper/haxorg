...
struct single_layout_run_state {
    ...
    single_layout_run_state(VertexID const& root_id, hstd::SPtr<layout::LayoutRun> run)
        : run{run}, root_id{root_id}, root_group{run->getGroup<kw::KiwiGroup>(root_id)} {}
    ...
    void collect_solver_nodes(VertexID const& id) {
        ...
        if (group->hasAlgorithm() && id != root_id) {
            ...
            if (auto kiwi_attr = run->getVertex(id)
                                     ->getOptionalAttribute<kw::KiwiVertexAttribute>()) {
        ...
        for (auto const& vert : run->getDirectVertices(id)) {
            ...
            auto attr = run->getVertexVisualAttribute<kw::KiwiVertexAttribute>(vert);
            solver_nodes.insert(vert);
            kiwi_rects.push_back(attr->rect);
        }

        for (auto const& sub : run->getSubGroups(id)) { collect_solver_nodes(sub); }
    }
    ...
    void collect_constraints(VertexID const& id) {
        ...
        for (auto const& sub : run->getSubGroups(id)) { collect_constraints(sub); }
    }
    ...
    geometry::Rect collect_bbox(VertexID const& id) {
        ...
        for (auto const& sub : run->getSubGroups(id)) { bbox.extend(collect_bbox(sub)); }
        ...
        for (auto const& vert : run->getDirectVertices(id)) {
            ...
            result.vertices.insert_or_assign(
                ...
                std::make_shared<kw::KiwiVertexLayoutAttribute>(
                    rect, run->getGraph()->getVertex(vert)->getStableId()));
        }

        for (auto const& sub : run->getSubGroups(id)) {
            auto sub_bbox_abs = bbox_map.at(sub);
            auto sub_bbox_rel = sub_bbox_abs.move(offset);
            auto sub_group    = run->getGroup<layout::IGroupVisualAttribute>(sub);

            if (sub_group->hasAlgorithm() && sub != root_id) {
                auto prev = run->getLayout<layout::IGroupLayoutAttribute>(sub);
                if (auto pad = sub_group->getOuterPadding()) {
                    sub_bbox_rel = sub_bbox_rel.withInnerPadding(pad.value());
                }
    ...
    if (align) {
        if (dimension == kiwi_ir::Axis::X) {
            LOGIC_ASSERTION_CHECK_FMT(
                align == kiwi_ir::Anchor::LEFT || align == kiwi_ir::Anchor::RIGHT
            ...
            LOGIC_ASSERTION_CHECK_FMT(
                align == kiwi_ir::Anchor::TOP || align == kiwi_ir::Anchor::BOTTOM
...
hstd::Vec<hstd::SPtr<kiwi_ir::ConstraintBase>> kw::AlignConstraint::getKiwi() const {
    if (vertices.empty()) {
        throw layout::layout_error::init("AlignConstraint expects vertices");
    }

    hstd::Vec<kiwi_ir::AlignItem> items;
    for (auto const& [id, spec] : vertices) {
        items.push_back(kiwi_ir::AlignItem{.rect_id = rectId(id), .spec = spec});
    }

    return {
        std::make_shared<kiwi_ir::AlignConstraint>(items, kiwi_ir::Strength::REQUIRED)};
}

hstd::Vec<hstd::SPtr<kiwi_ir::ConstraintBase>> kw::SeparateConstraint::getKiwi() const {
...
hstd::Vec<hstd::SPtr<kiwi_ir::ConstraintBase>> kw::MultiSeparateConstraint::getKiwi()
    const {
    if (lines.size() < 2) {
        throw layout::layout_error::init(
    ...
    for (auto const& lane : lines) {
        ...
        for (auto const& id : lane.getAllVertices()) {
            lane_ids.push_back(
                kiwi_ir::RectSpec1Side(rectId(id), toCenterAnchor(dimension)));
        }
...
layout::IPlacementAlgorithm::Result kw::KiwiLayoutAlgorithm::runSingleLayout(
    ...
    state.result.vertices.insert_or_assign(
        root_id,
        std::make_shared<kw::KiwiGroupLayoutAttribute>(root_bbox, state.root_group));
...
hstd::ext::visual::VisGroup kw::KiwiGroupLayoutAttribute::getVisual(
    ...
    rect_shape.geometry = Rect(0, 0, this->rect.width(), this->rect.height());
    ...
    res.elements.push_back(
        visual::VisElement::FromText(
            hstd::fmt("GROUP:{}", group->local.name), geometry::Point(0, 0)));

    res.custom.setAttr("inkscape:label", hstd::fmt("KIWI GROUP:{}", group->local.name));

    return res;
}

hstd::Vec<hstd::SPtr<kiwi_ir::ConstraintBase>> kw::ParentWrapConstraint::getKiwi() const {
    if (nested.empty()) {
        throw layout::layout_error::init("ParentWrapContraint expects vertices");
    }
...
hstd::Vec<hstd::SPtr<kiwi_ir::ConstraintBase>> kw::RelativeConstraint::getKiwi() const {
    return {
...
hstd::Vec<hstd::SPtr<kiwi_ir::ConstraintBase>> kw::EvenGapConstraint::getKiwi() const {
    return {
        std::make_shared<kiwi_ir::EvenGapConstraint>(
            vertices
                | rv::transform([this](VertexID const& id) -> kiwi_ir::RectSpec2Side {
                      return axis == kiwi_ir::Axis::X
                               ? kiwi_ir::RectSpec2Side::HorizontalRectBounds(rectId(id))
                               : kiwi_ir::RectSpec2Side::VerticalRectBounds(rectId(id));
                  })
                | rs::to<Vec>(),
            /*strength=*/strength),
    };
...
hstd::Vec<hstd::SPtr<kiwi_ir::ConstraintBase>> kw::EqualSizeConstraint::getKiwi() const {
    return {
