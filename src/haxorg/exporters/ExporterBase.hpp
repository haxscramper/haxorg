...
#define __obj_field(res, obj, name) visitFieldRedirect(res, #name, obj.name);

#define __org_field(res, obj, name) visitFieldRedirect(res, #name, obj->name);
...
void org::algo::Exporter<V, R>::visitField(
    ...
    auto __scope = trace_scope(
        trace(VisitReport::Kind::VisitField).with_field(name).with_node(org));
    _this()->visit(arg, org);
}

template <typename V, typename R>
void org::algo::Exporter<V, R>::visitSubnode(R& tmp, int, sem::SemId<sem::Org> val) {
    _this()->visit(tmp, val);
}

template <typename V, typename R>
void org::algo::Exporter<V, R>::visitDispatch(R& res, sem::SemId<sem::Org> arg) {
