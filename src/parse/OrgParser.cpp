...
#define CASE_SINGLE(Kind)                                                 \
    ...
    while (!lex.finished()) {
        switch (lex.kind()) {
            ...
            case otk::Whitespace: token(org::Space, pop(lex, lex.kind()));
            case otk::BraceEnd:
            case otk::ParEnd:
            case otk::ParBegin:
...
OrgId OrgParser::parseHashTag(OrgLexer& lex) {
    ...
    if (lex.at(otk::At)) {
        ...
        token(
            org::RawText, pop(lex, OrgTokSet{otk::HashIdent, otk::Word}));
    }

    if (lex.at(otk::DoubleHash)) {
        skip(lex, otk::DoubleHash);
        if (lex.at(OrgTokSet{otk::HashIdent, otk::Word})) {
            parseHashTag(lex);
...
OrgId OrgParser::parseListItem(OrgLexer& lex) {
    ...
    // prefix, 0
    {
        token(
            org::RawText,
            pop(lex, OrgTokSet{otk::ListItemBegin, otk::TreeClock}));
    }
    space(lex);
    // counter, 1
    {
...
OrgId OrgParser::parseList(OrgLexer& lex) {
    ...
    auto __trace = trace(lex);
    start(org::List);
    while (lex.at(OrgTokSet{otk::ListItemBegin, otk::TreeClock})) {
        parseListItem(lex);
...
OrgId OrgParser::parseSubtreeTimes(OrgLexer& lex) {
    ...
    auto __trace = trace(lex);
    start(org::StmtList);
    if (lex.ahead(OrgTokSet{otk::Whitespace}, OrgTokSet{otk::Time})) {
        skip(lex, otk::Whitespace);

        while (lex.at(otk::Time)) {
            start(org::TimeAssoc);
            if (lex.at(otk::Time)) {
                token(org::BigIdent, pop(lex, otk::Time));
            } else {
                empty();
            }
            space(lex);
