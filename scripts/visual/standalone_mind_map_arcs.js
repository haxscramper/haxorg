...
d3.json("/tmp/mindmap_graph.json").then(
  function (data) {
    ...
    const y = d3.scalePoint(nodes.map(d => Number(d.id)).sort(d3.ascending), [margin.top, height - margin.bottom]);
    ...
    const label = svg.append("g")
      ...
      .data(nodes)
      .join("g")
      .attr("transform", d => `translate(${margin.left},${d.y = y(Number(d.id))})`)
