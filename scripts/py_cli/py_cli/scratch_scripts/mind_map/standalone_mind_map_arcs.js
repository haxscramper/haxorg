...
d3.json("/tmp/mindmap_graph.json")
    .then(
        function(data) {
          ...
          const y =
              d3.scalePoint(nodes.map(d => Number(d.id)).sort(d3.ascending),
                            [ margin.top, height - margin.bottom ]);
          const color = d3.scaleOrdinal(
              nodes.map(d => d.group).sort(d3.ascending), d3.schemeCategory10)
          ...
          const label =
              svg.append("g")
                  ...
                  .call(g => g.append("text")
                                 ...
                                 .attr("dy", "0.35em")
                                 .attr("fill",
                                       d => d3.lab(color(d.group)).darker(2))
                                 .text(d => d.metadata.title))
                  .call(g => g.append("circle").attr("r", 3).attr(
