...
class WeekView(QWidget):
    ...
    def _get_week_events(self) -> List[CalendarEvent]:
        ...
        for event in self.events:
            ...
            if start_time and start_time.date() <= week_end and start_time.date(
            ) >= self.current_week_start:
                week_events.append(event)
            elif end_time and end_time.date() <= week_end and end_time.date(
            ) >= self.current_week_start:
                week_events.append(event)
    ...
    def _calculate_overlaps(
            self, events: List[CalendarEvent]) -> Dict[CalendarEvent, Tuple[int, int]]:
        overlaps = {}
    ...
    def _draw_event(self, painter: QPainter, event: CalendarEvent, day_index: int,
                    overlap_info: Tuple[int, int]) -> None:
        start_time = event.get_start_time()
        end_time = event.get_end_time()
        ...
        start_y = self.header_height + start_time.hour * self.hour_height + (
            start_time.minute * self.hour_height // 60)

        if event.has_duration() and end_time:
            if event.spans_multiple_days():
                self._draw_multi_day_event(painter, event, event_x, event_width, start_y,
                                           day_index)
            else:
        ...
        painter.drawText(event_x + 2, start_y + 12,
    ...
    def _draw_multi_day_event(self, painter: QPainter, event: CalendarEvent, x: int,
                              width: int, y: int, day_index: int) -> None:
        start_time = event.get_start_time()
        end_time = event.get_end_time()
