...
struct VisualDiagramNode
    ...
    QRectF boundingRect() const override {
        ...
        for (const auto& arrow : arrows) {
            QRectF arrowRect{arrow.p1(), arrow.p2()};
            rect = rect.united(arrowRect.normalized());
        }
    ...
    void paintArrows(QPainter* painter) {
        ...
        for (const auto& arrow : arrows) {
            ...
            QPointF direction = arrow.p2() - arrow.p1();
            if (direction.x() != 0 || direction.y() != 0) {
                direction = direction
                          / std::sqrt(
                                direction.x() * direction.x()
                                + direction.y() * direction.y());
                QPointF arrowHead1 = arrow.p2() - direction * 10
                                   + QPointF{
                                       -direction.y() * 5,
                                       direction.x() * 5};
                QPointF arrowHead2 = arrow.p2() - direction * 10
                                   + QPointF{
                                       direction.y() * 5,
                                       -direction.x() * 5};
                painter->drawLine(arrow.p2(), arrowHead1);
                painter->drawLine(arrow.p2(), arrowHead2);
            }
        }
    }
    ...
    void addArrow(const QPointF& target) {
        QPointF start = bounds.center();
        arrows.push_back(QLineF{start, target - pos()});
        update();
    }

    void updateArrows() {
        for (auto& arrow : arrows) { arrow.setP1(bounds.center()); }
        update();
    }
    ...
    QVariant itemChange(GraphicsItemChange change, const QVariant& value)
        override {
        if (change == ItemPositionHasChanged) { updateArrows(); }
...
struct DiagramScene : public QGraphicsScene {
  ...
  public:
    ...
    void setupExampleDiagram() {
        ...
        node1->addArrow(QPointF{250 + 50, 150 + 30});
    ...
    void mousePressEvent(QGraphicsSceneMouseEvent* event) override {
        ...
        VisualDiagramNode* newSelectedNode = dynamic_cast<
            VisualDiagramNode*>(item);
        ...
        if (event->button() == Qt::RightButton && selectedNode) {
            if (arrowSource == nullptr) {
                arrowSource = selectedNode;
            } else {
                if (arrowSource != selectedNode) {
                    QPointF targetPos = selectedNode->pos()
                                      + selectedNode->bounds.center();
                    arrowSource->addArrow(targetPos);
                }
                arrowSource = nullptr;
            }
        } else {
  ...
  public:
    ...
    void setupUI() {
        ...
        statusLabel = new QLabel{
            "Right-click to create arrows. Left-click to select and "
            "move."};
